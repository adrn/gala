...
static double hinit (unsigned n, FcnEqDiff fcn, CPotential *p, unsigned norbits,
        double x, double* y,
...
/* core integrator */
static int dopcor (unsigned n, FcnEqDiff fcn, CPotential *p, unsigned norbits,
		   ...
		   double beta, double fac1, double fac2, unsigned* icont)
{
  ...
  hlamb = 0.0;
  iasti = 0;
  fcn (n, x, y, k1, p, norbits);
  hmax = fabs (hmax);
  iord = 8;
  if (h == 0.0)
    h = hinit (n, fcn, p, norbits, x, y, posneg, k1, k2, k3, iord, hmax, atoler, rtoler, itoler);
  nfcn += 2;
  reject = 0;
  xold = x;
...
/* front-end */
int dop853
 (unsigned n, FcnEqDiff fcn, CPotential *p, unsigned norbits,
  ...
  long nmax, int meth, long nstiff, unsigned nrdens, unsigned* icont, unsigned licont)
{
  ...
  {
    idid = dopcor (n, fcn, p, norbits, x, y, xend, hmax, h, rtoler, atoler, itoler, fileout,
