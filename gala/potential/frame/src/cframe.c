...
double frame_hamiltonian(CFrame *fr, double t, double *qp, int n_dim) {
    double v = (fr->energy)(t, (fr->parameters), qp, n_dim);
    return v;
}

void frame_gradient(CFrame *fr, double t, double *qp, int n_dim, double *dH) {
    (fr->gradient)(t, (fr->parameters), qp, n_dim, dH);
}

void frame_hessian(CFrame *fr, double t, double *qp, int n_dim, double *d2H) {
