...
cpdef mockstream_dop853_snapshot(nbody, double[::1] time,
                                 ...
                                 int check_filesize=1, int overwrite=0):
    """
    Parameters
    ----------
    nbody : `~gala.dynamics.nbody.DirectNBody`
    ...
    _nbody_out = dop853_helper_save_all(&cp, &cf,
                                        <FcnEqDiff> Fwrapper_direct_nbody,
                                        nbody_w0, output_times,
                                        ndim, nbodies, args, noutput_times,
                                        atol, rtol, nmax)
    nbody_out = np.rollaxis(np.array(_nbody_out), -1)

    # Create the file and store the nbody orbits:
    ...
    for i in range(ntimes):
        ...
        while tmp_t2 < t2:
            ...
            h5f['stream/pos'][:, j, cumn[i]:cumn[i]+nstream[i]] = np.array(w_tmp)[nbodies:nbodies+nstream[i]].T[:3]
            h5f['stream/vel'][:, j, cumn[i]:cumn[i]+nstream[i]] = np.array(w_tmp)[nbodies:nbodies+nstream[i]].T[3:]
