...
coordinate system, like the ICRS. The resulting system is aligned so that the
north pole is the pole of a specified great circle, with additional
specifications for the longitude zero point of the final system.

Currently, ``gala`` only supports great circle frames that are defined as a
rotation away from the ICRS (ra, dec) system through the
