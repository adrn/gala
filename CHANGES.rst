...
- Added a new method ``MockStream.rotate_to_xy_plane()`` that transforms a mock stream
  into a new coordinate system where the progenitor's orbital plane is aligned with the
  xy-plane, the stream and progenitor are centered at (0, 0), and the stream primarily
  extends in the x direction (leading tail at positive x and trailing tail at negative
  x).
