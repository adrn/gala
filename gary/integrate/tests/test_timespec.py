...
def test_dt_t1_t2():
    ...
    t = parse_time_specification(dt=0.1, t1=10., t2=130.)
    np.testing.assert_allclose(np.min(t), 10.)
    np.testing.assert_allclose(np.max(t), 130.)

    t = parse_time_specification(dt=-0.1, t1=10., t2=-13.412)
    np.testing.assert_allclose(np.min(t), -13.412)
    np.testing.assert_allclose(np.max(t), 10.)

    with pytest.raises(ValueError):
        parse_time_specification(dt=-0.1, t1=10., t2=130.)
...
def test_t():
    ...
    t = parse_time_specification(t=input)
    assert (t == input).all()
